function [fi,dx,dy,ddx,ddy]=rbfDerivative(cloud,unkno)
...
fi = rbf_interp_nd ( m, nd, cloud, r0, @phi1, w, 1, cloud(:,1) ); %interpolated solution
%RBF_WEIGHT - computes the rbf weight values
w = rbf_weight ( m, nd, cloud, r0, @phi1, fx );
%RBF_INTERP_ND - does the RBF interpolation
dx = rbf_interp_nd ( m, nd, cloud, r0, @phi1, w, 1, cloud(:,1) ); %first x-derivative
%RBF_WEIGHT - computes the rbf weight values
w = rbf_weight ( m, nd, cloud, r0, @phi1, fxx );
%RBF_INTERP_ND - does the RBF interpolation
ddx = rbf_interp_nd ( m, nd, cloud, r0, @phi1, w, 1, cloud(:,1) ); %second x-derivative
%RBF_WEIGHT - computes the rbf weight values
w = rbf_weight ( m, nd, cloud, r0, @phi1, fy );
%RBF_INTERP_ND - does the RBF interpolation
dy = rbf_interp_nd ( m, nd, cloud, r0, @phi1, w, 1, cloud(:,1) ); %first y-derivative
%RBF_WEIGHT - computes the rbf weight values
w = rbf_weight ( m, nd, cloud, r0, @phi1, fyy );
%RBF_INTERP_ND - does the RBF interpolation
ddy = rbf_interp_nd ( m, nd, cloud, r0, @phi1, w, 1, cloud(:,1) ); %second x-derivative